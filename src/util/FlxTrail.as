package util
{
	...
	{		
		...
		private var newTrail:Boolean = true;
		
		private static const TRAIL_SIZE:uint = 50;
		 ...
		 */
		override public function FlxTrail(Sprite:FlxSprite = null, Image:Class = null, Length:int = 10, Delay:int = 3, Alpha:Number = 0.4, Diff:Number = 0.05):void
		{
			// Sync the vars 
			sprite = Sprite;
			delay = Delay;
		...
		public function getBulletTrail(Sprite:FlxSprite):FlxTrail
		{
			...
			{
				delay = 1;
				image = AssetsRegistry.bulletTrailPNG;
				transp = 1;
				difference = 0;
				rotationsEnabled = false;
				
				// Create the initial trailsprites
				increaseLength(TRAIL_SIZE);
		...
		override public function update():void
		{
			...
			if (this.members[0].x == this.members[TRAIL_SIZE - 1].x && this.members[0].y == this.members[TRAIL_SIZE - 1].y)
			{
				this.exists = false;
			}
		...
		public function increaseLength(amount:int):void
		{
			...
			for (var i:int = 0; i < amount; i++) {
				...
				{
					//positions initial appearance away from screen
					trailSprite.x = -(trailSprite.width);
					trailSprite.y = -(trailSprite.height);
				}
